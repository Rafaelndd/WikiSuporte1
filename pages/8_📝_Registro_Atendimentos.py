...
with tab_lancar:
    ...
    with st.expander("⚙️ Gerenciar categorias de atendimento"):
        st.caption("Adicione novas categorias para uso no formulário de atendimento abaixo.")
        cat_c1, cat_c2 = st.columns([3, 1], vertical_alignment="bottom")
        nova_categoria = cat_c1.text_input("Nova categoria", key="p8_nova_categoria_dev")
        if cat_c2.button("Adicionar", key="p8_add_categoria_dev", use_container_width=True):
    ...
    with st.container(border=True):
        st.markdown("##### 🗒️ Dados do atendimento")
